...
parser.add_argument('--name', type=str, default='', help='What to name output')
...
if args.evaluate:
  ...
  avg_reward, avg_Q = test(args, 0, dqn, val_mem, args.name, evaluate=True)  # Test
  ...
  for T in tqdm(range(args.T_max)):
    ...
    if T >= args.learn_start:
      ...
      if T % args.evaluation_interval == 0:
        dqn.eval()  # Set DQN (online network) to evaluation mode
        avg_reward, avg_Q = test(args, T, dqn, val_mem, args.name)  # Test
