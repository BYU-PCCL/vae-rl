...
class VLadder(Network):
    def __init__(self, dataset, name=None, reg='kl', batch_size=100, restart=False):
        ...
        self.class_label_placeholder = tf.placeholder(dtype=tf.int32, shape=[None], name='class_label_placeholder')
        ...
        if self.ladder0_dim > 0:
            self.iladder0_mean, self.iladder0_stddev = layers.ladder0(self.input_placeholder, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
        ...
        if self.num_layers >= 2:
            self.ilatent1_hidden = layers.inference0(self.input_placeholder, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            if self.ladder1_dim > 0:
                self.iladder1_mean, self.iladder1_stddev = layers.ladder1(self.ilatent1_hidden, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
        ...
        if self.num_layers >= 3:
            self.ilatent2_hidden = layers.inference1(self.ilatent1_hidden, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            if self.ladder2_dim > 0:
                self.iladder2_mean, self.iladder2_stddev = layers.ladder2(self.ilatent2_hidden, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
        ...
        if self.num_layers >= 4:
            self.ilatent3_hidden = layers.inference2(self.ilatent2_hidden, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            if self.ladder3_dim > 0:
                self.iladder3_mean, self.iladder3_stddev = layers.ladder3(self.ilatent3_hidden, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
        ...
        if self.ladder0_dim > 0:
            ...
            self.tlatent0_state = layers.generative0(None, self.iladder0_sample, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            self.glatent0_state = layers.generative0(None, self.ladder0_placeholder, self.class_label_placeholder, self.n_classes, reuse=True, is_training=False)
        else:
            tlatent_0_state, glatent0_state = None, None
        ...
        if self.num_layers >= 2 and self.ladder1_dim > 0:
            ...
            tlatent1_state = layers.generative1(tlatent0_state, self.iladder1_sample, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            glatent1_state = layers.generative1(glatent0_state, self.ladder1_placeholder, self.class_label_placeholder, self.n_classes, reuse=True, is_training=False)
        elif tlatent0_state is not None:
            tlatent1_state = layers.generative1(tlatent0_state, None, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            glatent1_state = layers.generative1(glatent0_state, None, self.class_label_placeholder, self.n_classes, reuse=True, is_training=False)
        else:
        ...
        if self.num_layers >= 3 and self.ladder2_dim > 0:
            ...
            tlatent2_state = layers.generative2(tlatent1_state, self.iladder2_sample, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            glatent2_state = layers.generative2(glatent1_state, self.ladder2_placeholder, self.class_label_placeholder, self.n_classes, reuse=True, is_training=False)
        elif tlatent1_state is not None:
            tlatent2_state = layers.generative2(tlatent1_state, None, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            glatent2_state = layers.generative2(glatent1_state, None, self.class_label_placeholder, self.n_classes, reuse=True, is_training=False)
        else:
        ...
        if self.num_layers >= 4 and self.ladder3_dim > 0:
            ...
            tlatent3_state = layers.generative3(tlatent2_state, self.iladder3_sample, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            glatent3_state = layers.generative3(glatent2_state, self.ladder3_placeholder, self.class_label_placeholder, self.n_classes, reuse=True, is_training=False)
        elif tlatent2_state is not None:
            self.toutput = layers.generative3(tlatent2_state, None, self.class_label_placeholder, self.n_classes, is_training=self.is_training)
            self.goutput = layers.generative3(glatent2_state, None, self.class_label_placeholder, self.n_classes, reuse=True, is_training=False)
        else:
    ...
    def train(self, batch_input, class_labels, batch_target, label=None):
        self.iteration += 1
        ...
        feed_dict.update({
            ...
            self.class_label_placeholder: class_labels,
    ...
    def test(self, batch_input, class_labels, label=None):
    ...
    def inference(self, batch_input, class_labels):
        tensor_handle = [self.ladders[key][2] for key in self.ladders]
        tensor_value = self.sess.run(tensor_handle, feed_dict={self.input_placeholder: batch_input,
                                                               self.class_label_placeholder: class_labels,
                                                               self.is_training: False})
    ...
    def generate(self, codes):
        feed_dict = {self.ladders[key][0]: codes[key] for key in self.ladders}
        feed_dict_keys = feed_dict.keys()
        batch_size = feed_dict[feed_dict_keys[0]].shape[0]
        feed_dict[self.is_training] = False
        feed_dict[self.class_label_placeholder] = np.random.randint(self.n_classes, size=[batch_size])
    ...
    def generate_conditional_samples(self, condition_layer, condition_code, batch_size=None, class_label=None):
        codes = {key: np.random.normal(size=[self.batch_size, self.ladders[key][1]]) for key in self.ladders}
        ...
        if class_label is None:
            feed_dict[self.class_label_placeholder] = list(range(self.n_classes))*(batch_size//self.n_classes + 1)[:batch_size]
        else:
            feed_dict[self.class_label_placeholder] = [class_label] * batch_size
        feed_dict[self.class_label_placeholder] = np.random.randint(self.n_classes, size=[self.batch_size])
        output = self.sess.run(self.goutput, feed_dict=feed_dict)
        return output[random_indexes], condition_code

    def generate_samples(self, batch_size=None, class_label=None):
    ...
    def generate_manifold_samples(self, external_layer, external_code):
        ...
        feed_dict = {self.ladders[key][0]: codes[key] for key in self.ladders}
        feed_dict[self.is_training] = False
        feed_dict[self.class_label_placeholder] = np.random.randint(self.n_classes, size=[external_code.shape[0]])
        output = self.sess.run(self.goutput, feed_dict=feed_dict)
        return output
