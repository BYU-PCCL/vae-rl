...
class ConditionalSampleVisualizer(Visualizer):
    ...
    def visualize(self, layers, num_rows=4, use_gui=False):
        ...
        for i, layer in enumerate(layers):
            samples = np.zeros([self.num_classes*num_rows]+self.dataset.data_dims)
            samples_ptr = 0
            for j in range(self.num_classes):
                ...
                new_samples, _ = self.network.generate_conditional_samples(layer, latent_code, batch_size=num_rows, class_label=j)
                next_ptr = samples_ptr + new_samples.shape[0]
                if next_ptr > self.num_classes * num_rows:
                    next_ptr = self.num_classes * num_rows
